...
_ASANA_TIMEFRAMES = [_ASANA_TIMEFRAME_COMPLETED, _ASANA_TIMEFRAME_NOT_COMPLETED, "all"]
...
PLATFORM_SCHEMA = config_validation.PLATFORM_SCHEMA.extend(
    {
        ...
        voluptuous.Optional(
            const.CONF_NAME, default=_DEFAULT_NAME
    ...
    def get_api_data(self):
        ...
        while api_url:
            response = requests.get(
                api_url, headers={"Authorization": "Bearer {}".format(self._token)}
            )
...
class AsanaTaskSensor(entity.Entity):
    ...
    def _get_days_from_monitored_variable(self, monitored_variable):
        ...
        match_days = re.search(
            r"(?:_)([\d]+)(?:day)", monitored_variable, re.IGNORECASE
        )
        if not match_days:
            return None
    ...
    def _task_day_matches_timeframe(self, task_day, variable_timeframe, variable_date):
    ...
    def _store_task_data(self, tasks_data):
        ...
        for task in tasks_data:
            ...
            self._task_data[task_group][task_date].append(task.get("name"))
    ...
    def _update_sensor_attributes(self):
        ...
        for variable_name, variable_config in self._monitored_variables.items():
            ...
            self._attributes[variable_name] = (
                matching_task_list
                if variable_type == _ASANA_SENSOR_TYPE_LIST
